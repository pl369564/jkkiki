...
namespace CToool
{
    public static class MathTool
    {
        ...
        /// 计算加加速度运动距离
        /// </summary>
        public static double jSpeedMotion(double j,double t)
        {
            //a = j * t;
            //v = (j * t / 2) * t;
            //s = (j * t / 2 * t) * t / 3
        ...
        public static double SquaredDistance(double ax, double ay, double az
        , double bx, double by, double bz)
        {
            double num = ax - bx;
            double num2 = ay - by;
    ...
    public static class CRC
    {
        ...
        public static UInt16 crc16(byte[] buffer, UInt16 start, UInt16 length)
        {
            UInt16 crc = 0;

            for (int i = start; i < start + length; i++)
            {
                crc = (UInt16)((crc << 8) ^ crc16tab[((crc >> 8) ^ buffer[i]) & 0x00FF]);
